#' Extract tables from one column pdf documents in a folder
#'
...
#' @export
directory_tables <- function(directory,
                             rec = FALSE,
                             file_pattern= ".pdf",
                             full_names = TRUE,
                             recursive = FALSE,
  ...
  tables <- lapply(seq_along(files_dir), function(xx)
    extractor_tables(files_dir[[xx]], path = TRUE,
                     rec = rec,
                     delimiter = "\\s{2,}",
                     delimiter_table = "\\s{2,}",
                     replacement = "|"))

  names(tables) <- file_name
